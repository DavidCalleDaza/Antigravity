...
export default function ServiceFormDrawer({
  ...
  useEffect(() => {
    if (isOpen) {
      setOpenSection(editingService ? 'info' : 'media');
      setAudioError('');
      setVideoError('');
    }
  }, [isOpen, editingService]);

  const saveActionTitle = editingService ? 'Guardar Cambios' : 'Crear Servicio';
  ...
  const mainMediaCount = preview || formData.image_url || editingService?.image_url ? 1 : 0;
  ...
  return (
    ...
    >
      <form id="service-form-drawer" className="d-flex flex-col gap-4" onSubmit={onSubmit}>
        {/* ── 1. MULTIMEDIA ── */}
        <AccordionSection
